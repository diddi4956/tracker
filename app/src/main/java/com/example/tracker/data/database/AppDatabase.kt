...
import com.example.tracker.data.dao.ExpenseCategoryDefinitionDao
...
@Database(// 되게 신기한게 클래스로 시작되지가 않는다 이건? 왜지? 이 어노테이션은 뭐지
    entities = [ExpenseCategoryDefinition::class,
...
abstract class AppDatabase : RoomDatabase(){ // 인터페이스랑의 차이가 뭐지?
    abstract fun expenseCategoryDefinitionDao(): ExpenseCategoryDefinitionDao
