...
interface ConditionRecordDao {
    ...
    suspend fun trackingByDefinition(start:String, end:String): List<ConditionTrackingByDefinitionDto>

    @Query("SELECT d.name AS conditionDefinitionName, d.id AS definitionId, t.id AS tagId, t.name AS tagName " +
    ...
    @Query("SELECT COUNT(r.id) AS checkedCount, r.date AS date, r.conditionDefinitionId AS conditionDefinitionId, d.name AS definitionName, t.name AS tagName, d.conditionCategoryId AS conditionCategoryId " +
            ...
            "ORDER BY d.name ASC")
    suspend fun trackingByTag(tagIds: List<Long>, start: String, end: String): List<ConditionTrackingByTagDto>
    ...
    @Query("SELECT r.date AS date, COUNT(r.id) AS countOfRecord " +
            ...
            "GROUP BY r.date")
    suspend fun getMonthlyByTag(tagId: Long, start: String, end: String): List<ConditionGetMonthlyByTagDto>

    // 데일리 체크리스트 -> /dto 수정필요
    @Query("SELECT d.id AS id, t.id AS tagId, d.name AS name, t.name AS tagName, CASE WHEN r.id IS NULL THEN 0 ELSE 1 END AS checked " +
            ...
            "WHERE t.id IN (:tagIds)" +
            "ORDER BY t.name, d.name") // 데피니션이 최소 태그 하나는 갖게하고싶은데...엔티티를 어떻게 해야할까 -> 엔티티보단 트랜잭션으로 묶기
    ...
    @Transaction
    suspend fun addRecordAndPlusDefinitionFrequency(record: ConditionCheckRecord){
        insert(record)
    ...
    @Transaction
    suspend fun deleteRecordAndSubtractionFrequency(record: ConditionCheckRecord){
        delete(record)
        val definition = getDefinitionById(record.conditionDefinitionId)
        updateDefinition(definition.copy(frequency = definition.frequency -1))
    }
